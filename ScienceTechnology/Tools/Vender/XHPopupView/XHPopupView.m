...
- (void)containsPopupBlock:(void (^)(XHPopupView *popup))block;
- (void)dismissShowingPopup:(BOOL)animated;
@end

@interface XHPopupView ()
...
+ (XHPopupView *)popupWithContentView:(UIView *)contentView {
    XHPopupView *popup = [[[self class] alloc] init];
...
+ (void)dismissAllPopups {
    ...
    for (UIWindow *window in windows) {
        [window containsPopupBlock:^(XHPopupView * _Nonnull popup) {
    ...
    if (!_isBeingShown && !_isShowing && !_isBeingDismissed) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (strongSelf.maskType == XHPopupMaskType_Dimmed) {
                ...
                strongSelf.backgroundView.backgroundColor = [UIColor.blackColor colorWithAlphaComponent:0.3];
            }
...
- (void)containsPopupBlock:(void (^)(XHPopupView *popup))block {
    for (UIView *subview in self.subviews) {
        if ([subview isKindOfClass:[XHPopupView class]]) {
            block((XHPopupView *)subview);
        } else {
...
- (void)dismissShowingPopup:(BOOL)animated {
    UIView *view = self;
    while (view) {
        if ([view isKindOfClass:[XHPopupView class]]) {
            [(XHPopupView *)view dismissAnimated:animated];
            break;
        }
        view = view.superview;
    }
