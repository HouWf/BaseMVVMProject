...
- (void)itemDidLayoutBadgeLabel{
    switch (self.itemModel.itemBadgeStyle) {
        case AxcAE_TabBarItemBadgeStyleTopRight:{   // 右上方 默认
            self.badgeLabel.center = CGPointMake(self.frame.size.width/2 + 15,self.badgeLabel.frame.size.height/2 + 3);
        }break;
...
//    if (itemModel.bulgeStyle == AxcAE_TabBarConfigBulgeStyleCircular) {
//        self.icomImgView.layer.shadowOffset = CGSizeMake(0, 0);
//        self.icomImgView.layer.shadowColor = UIColor.lightGrayColor.CGColor;
//        self.icomImgView.layer.shadowOpacity = 0.6;
//    }
...
- (AxcAE_TabBarBadge *)badgeLabel{
    if (!_badgeLabel) {
        ...
        _badgeLabel.layer.borderWidth = 2.f;
        _badgeLabel.layer.borderColor = UIColor.whiteColor.CGColor;
//        _badgeLabel.backgroundColor = COLOR_FF6666;
