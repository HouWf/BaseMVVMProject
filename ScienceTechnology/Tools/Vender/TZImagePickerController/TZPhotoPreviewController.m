...
- (void)viewDidLoad {
    ...
    if (_tzImagePickerVc.previewShowTopToolBar) {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        [self->_doneButton setTitleColor:_tzImagePickerVc.oKButtonTitleColorNormal forState:UIControlStateNormal];
    });
}
...
- (void)configBottomToolBar {
    ...
    if (!tzImagePickerVc.previewShowTopToolBar) {
        _toolBar.backgroundColor = [UIColor colorWithRed:rgb green:rgb blue:rgb alpha:0.1];
    }
    ...
    if (_tzImagePickerVc.allowPickingOriginalPhoto) {
        ...
        [_bottomCancelButton addTarget:self action:@selector(backButtonClick) forControlEvents:UIControlEventTouchUpInside];
        [_bottomCancelButton setTitle:@"取消" forState:UIControlStateNormal];
        [_bottomCancelButton setTitleColor:_tzImagePickerVc.oKButtonTitleColorNormal forState:UIControlStateNormal];
    ...
    [_toolBar addSubview:_bottomCancelButton];
...
- (void)viewDidLayoutSubviews {
    ...
    if (_tzImagePickerVc.allowPickingOriginalPhoto) {
        ...
        [_bottomCancelButton sizeToFit];
        _bottomCancelButton.frame = CGRectMake(12, 0, _bottomCancelButton.tz_width, 44);
    }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_tzImagePickerVc.allowPickingMultipleVideo && model.type == TZAssetModelMediaTypeVideo) {
        ...
        [photoPreviewCell.previewView setScrollBlock:^(BOOL ifAnimate) {
            self->_doneButton.enabled = !ifAnimate;
            if (ifAnimate) {
                [self->_doneButton setTitleColor:UIColor.lightGrayColor forState:UIControlStateNormal];
            }
            else{
                [self->_doneButton setTitleColor:_tzImagePickerVc.oKButtonTitleColorNormal forState:UIControlStateNormal];
            }
...
- (void)refreshNaviBarAndBottomBarState {
    ...
    if (![[TZImageManager manager] isPhotoSelectableWithAsset:model.asset]) {
        ...
        _bottomCancelButton.hidden = YES;
    }
}
