...
- (void)setIsHitTest:(BOOL)isHitTest
{
    objc_setAssociatedObject(self, @selector(isHitTest), @(isHitTest), OBJC_ASSOCIATION_ASSIGN);
}
...
/**
 *  根据range给文本添加点击事件delegate回调 hiteTest方式
 *
 *  @param strings  需要添加的Range字符串数组
 *  @param delegate delegate
 */
- (void)yb_addAttributeHitTestActionWithRanges:(NSArray <NSString *> *)strings
                                      delegate:(id <YBAttributeTapActionDelegate> )delegate{
    self.isHitTest = YES;
    [self yb_removeAttributeTapActions];
    [self yb_getRangesWithStrings:strings];
